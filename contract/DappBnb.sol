...
contract DappBooking is Ownable,ReentrancyGuard{
...
function createAppartment (
    ...
    require(price > 0 ether,'Proce cannot be zero');
...
for (uint i = 0; i < dates.length; i++) {
 ...
 function claimFunds(uint aid,uint bookingId) public nonReentrant{
    BookingStruct memory booking=bookingsOf[aid][bookingId];
    require(msg.sender==apartments[aid].owner || msg.sender == owner(),
   "Unauthorized entity" );
...
function getBookings(uint aid,uint  bookingId) public view returns (BookingStruct memory){
...
function getUnavaibaleDates(uint aid) public view returns (uint[] memory){
    return bookedDates[aid];
}


function addReview(uint aid, string memory comment) public {
    require(appartmentExist[aid],'Appartment not availabe');
